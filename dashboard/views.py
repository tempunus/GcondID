...
class DashboardView(ApprovedUserRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        tickets = _filter_by_user_condominiums(Ticket.objects.all(), user)
        stock_items = _filter_by_user_condominiums(StockItem.objects.all(), user)
        movements = StockMovement.objects.select_related("item", "item__condominium", "user")
        movements = _filter_movements_by_user_condominiums(movements, user)
        ticket_counts = dict(tickets.values_list("status").annotate(total=Count("id")))
        context.update(
            {
                "total_users": User.objects.count() if user.is_gcondid_admin else None,
